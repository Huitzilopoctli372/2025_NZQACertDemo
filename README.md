...
graph TD
    ...
    ODS_P & S_P & S_K -- CECode Transformation (LEFT/CASE), Key Project Description Lookup, Dead Row Deletion --> CP
...
graph TD
    ...
    S_B & ODS_P -- Extract/Join Financial Items (e.g., Budget, Actuals, Forecast) to Project IDs --> BB
    BB -- INSERT --> ODS_B
    
    ODS_B -- Standardize Budget Data (e.g., set defaults, clean categories) --> CB
    CB -- UPDATE/DELETE --> ODS_B
    
    ODS_B & S_I & ODS_P -- Divide Delivery Amounts by Inflation Rate (Inflation_Profile Join) --> UIDA
    UIDA -- Updates --> ODS_B
    
    ODS_B -- Calculate Capital Loan Revenue/Items based on Budgets --> BCL
    BCL -- INSERT/UPDATE --> ODS_B
    
    ODS_B & ODS_P -- Check Budget Totals, Financial Status against Project Status --> RVC
    RVC -- Inserts Errors --> ODS_V
```
